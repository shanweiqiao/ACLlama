...
def preprocess_caption(
    ...
    for source in sources:
        ...
        audio_array = item["audio"]["array"] if "audio" in item.keys() else None
        """
        asr
        """
        # caption question
        _input_id += [start_header_id] + _user + [end_header_id] + audio_placeholder_ids + tokenizer(asr_prompt).input_ids + [eot_id]
        _target += [IGNORE_TOKEN_ID] * len(_input_id)
        prefix_index+=(len(_input_id))
        ...
        # caption answer
        _input_id += [start_header_id] + _assistant + [end_header_id] + nl_tokens + tokenizer(item["speech_caption"]).input_ids + [eot_id] # 这个里面应该有个回答的前缀
        _target += [IGNORE_TOKEN_ID] + [IGNORE_TOKEN_ID] * len(_assistant) + \
                    [IGNORE_TOKEN_ID] + [IGNORE_TOKEN_ID] * len(nl_tokens) + tokenizer(value).input_ids + [eot_id]
        prefix_index+=(len([start_header_id] + _assistant + [end_header_id] + nl_tokens))
        #  TODO change the process code of ASR data
        asr_value = item["speech_caption"] if "speech_caption" in item.keys() else None
        if asr_value:
            asr_id = tokenizer(asr_value).input_ids[1:]
            asr_targets.append(asr_id+[IGNORE_TOKEN_ID] * (MAX_ASR_LENGTH - len(asr_id)))
        ...
        """
        # caption question
        _input_id += [start_header_id] + _user + [end_header_id] + audio_placeholder_ids + tokenizer(caption_prompt).input_ids + [eot_id]
        _target += [IGNORE_TOKEN_ID] * len(_input_id)
        prefix_index+=(len(_input_id))
        ...
        # caption answer
        _input_id += [start_header_id] + _assistant + [end_header_id] + nl_tokens + tokenizer(item["audio_caption"]).input_ids + [eot_id]
        _target += [IGNORE_TOKEN_ID] + [IGNORE_TOKEN_ID] * len(_assistant) + \
                    [IGNORE_TOKEN_ID] + [IGNORE_TOKEN_ID] * len(nl_tokens) + tokenizer(value).input_ids + [eot_id]
        prefix_index+=(len([start_header_id] + _assistant + [end_header_id] + nl_tokens))
        #  TODO change the process code of ASR data
        asr_value = item["audio_caption"] if "audio_caption" in item.keys() else None
        if asr_value:
            asr_id = tokenizer(asr_value).input_ids[1:]
...
class LazySupervisedDataset(Dataset):
    ...
    def _process_item(self, index):
        ...
        if self.raw_data_caption is not None:
            ret = preprocess_caption([self.raw_data_caption[index]], self.tokenizer, self.max_len)
        audio_path = ret["audio_paths"][0]
        audio, _ = librosa.load(audio_path, sr=CONFIG.sampling_rate)
        audio_feat = self.audio_processor(audio, sampling_rate=CONFIG.sampling_rate, return_tensors="pt").input_features
        audio_feat = audio_feat.squeeze(0).to(torch.bfloat16)
...
def make_supervised_data_module(
    ...
    ######## load caption data
    data = load_from_disk("/data/s50042884/huggingface_model/FusionAudio-1")
