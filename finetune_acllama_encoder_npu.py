...
def make_supervised_data_module(
    ...
    ######## load caption data
    data = load_from_disk("/data/s50042884/huggingface_model/FusionAudio-1-filter-en")
...
def train():
    ...
    #######-load stage1 model
    pretrained_model_path = "/data/s50042884/my_code/ACLlama_A100_right/ACLlama_output/ACLlama_encoder_stage1_chatllm/checkpoint-4110"
    combined_weights = torch.load(pretrained_model_path + "/base_model.bin", map_location=f"cpu")
    print(f"model is : {model}")
    # print(f"combined_weights is : {combined_weights.keys()}")
    ...
    for item in combined_weights.keys():
        ...
        need_combined_weights[fix_item] = combined_weights[item].to("npu")
    ...
    # pretrained_model_path = "/data/s50042884/my_code/ACLlama_A100_right/ACLlama_output/ACLlama_encoder_contrastive_base_stage1_chatllm_npu/checkpoint-1100"
    # combined_weights = torch.load(pretrained_model_path + "/base_model.bin", map_location=f"cpu")
    # model.load_state_dict(combined_weights, strict=True)
    #######
    ...
    with torch.autocast("npu"):
        # trainer.train(resume_from_checkpoint="/data/s50042884/my_code/ACLlama_A100_right/ACLlama_output/ACLlama_encoder_contrastive_base_stage1_chatllm_npu/checkpoint-1100")
        trainer.train()
    trainer.save_state()
