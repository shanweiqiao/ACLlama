...
def train():
    ...
    if training_args.use_lora:
        ...
        for id, (name, param) in enumerate(model.named_modules()):
            ...
            lora_list = ["q_proj", "k_proj"]
            language_model_lora_tag = any(item in name for item in lora_args.lora_target_modules)
            if language_model_lora_tag and 'model.layers.' in name:
                my_target_modules.append(name)
        # print(f"my_target_modules is : {my_target_modules}")
        # print(f"lora_args.lora_target_modules is : {lora_args.lora_target_modules}")
        # exit(0)
    ...
    # training_args = replace(training_args, deepspeed="/data/s50042884/my_code/ACLlamacode/my_accelerate.json")
