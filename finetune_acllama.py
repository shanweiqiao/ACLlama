...
def train():
    ...
    if training_args.use_lora:
        ...
        """
        pretrained_model_path = "/data/s50042884/my_code/ACLlama_output/ACLlama_encoder_finetune_contrastive_captiondata_large_batch_after_stage1/checkpoint-250"
        shard_state = torch.load(pretrained_model_path + "/base_model.bin", map_location=f"cuda")
        need_combined_weights = {}
        ...
        for item in shard_state.keys():
            ...
            if "model.audio_feature_head.weight" == item:
                fix_item = "base_model.model.model.audio_feature_head.weight"
                need_combined_weights[fix_item] = shard_state[item]
            if "lm_head.weight" == item:
                fix_item = "base_model.model.lm_head.weight"
                need_combined_weights[fix_item] = shard_state[item]
            if "model.norm.weight" == item:
                fix_item = "base_model.model.model.norm.weight"
                need_combined_weights[fix_item] = shard_state[item]
    ...
    with torch.autocast("cuda"):
        # trainer.train(resume_from_checkpoint="/data/s50042884/my_code/ACLlama_output/ACLlama_encoder_finetune_contrastive_captiondata_large_batch_after_stage1/checkpoint-400")
        trainer.train()
    trainer.save_state()
